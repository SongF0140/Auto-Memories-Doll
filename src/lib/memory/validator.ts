...
export const validatePendingEvent = (event: Partial<PendingEvent>): event is PendingEvent => {
  ...
  if (!event.sourceType || !["chat", "ingest", "manual", "mcp", "skill"].includes(event.sourceType)) return false;
  ...
  if (!event.status || !["pending", "processing", "done", "failed"].includes(event.status)) return false;
  if (event.retryCount === undefined || typeof event.retryCount !== "number") return false;
  return true;
};

export const validateConflictRecord = (record: Partial<ConflictRecord>): record is ConflictRecord => {
  ...
  if (!record.status || !["pending", "resolved_accept", "resolved_keep", "resolved_manual"].includes(record.status)) return false;
  if (!record.createdAt || typeof record.createdAt !== "string") return false;
  return true;
};