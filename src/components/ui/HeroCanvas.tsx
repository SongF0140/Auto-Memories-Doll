...
function MobileNodePattern() {
  return (
    ...
    >
      <style>{`
        .mobile-node-cluster-a { animation: mobile-node-drift-a 12s ease-in-out infinite alternate; transform-origin: 120px 210px; transform-box: fill-box; }
        .mobile-node-cluster-b { animation: mobile-node-drift-b 14s ease-in-out infinite alternate; transform-origin: 270px 350px; transform-box: fill-box; }
        .mobile-node-pulse { animation: mobile-node-pulse 3s ease-in-out infinite alternate; }
        @keyframes mobile-node-drift-a { from { transform: translate(-8px, 4px) rotate(-1deg); } to { transform: translate(10px, -7px) rotate(1.5deg); } }
        @keyframes mobile-node-drift-b { from { transform: translate(7px, -6px) rotate(1deg); } to { transform: translate(-10px, 8px) rotate(-1.4deg); } }
        @keyframes mobile-node-pulse { from { opacity: 0.52; } to { opacity: 0.8; } }
      `}</style>
...
export function HeroCanvas() {
  ...
  const drawFrame = useCallback(() => {
    ...
    // ========== 绘制网络连线（核心视觉效果）==========
    ctx.strokeStyle = `rgba(${fg}, 1)`; // 深棕色线条
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      for (let j = i + 1; j < nodes.length; j++) {
        const b = nodes[j];
        const dx = a.x - b.x;
        if (Math.abs(dx) >= config.maxDist) continue; // X轴快速排除

        const dy = a.y - b.y;
        if (Math.abs(dy) >= config.maxDist) continue; // Y轴快速排除

        const distSq = dx * dx + dy * dy;
        if (distSq >= maxDistSq) continue; // 超出最大距离
    ...
    if (isMobile && config.traceAlpha > 0) {
      ...
      for (const n of nodes) {
        ...
        traces += 1;
        if (traces >= 6) break; // 限制追踪线数量
      }
    }

    // ========== 绘制节点（圆点）==========
    ctx.globalAlpha = 1;
    ctx.fillStyle = `rgba(${fg}, ${config.nodeAlpha})`; // 深棕色半透明节点
  ...
  }, [updateRenderMode, resizeDebounceMs]);
