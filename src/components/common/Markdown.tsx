...
export default function Markdown({ content, className = "" }: MarkdownProps) {
  ...
  const flushParagraph = () => {
    ...
    blocks.push(
      <p key={`p-${blocks.length}`} className="mb-4 leading-8">
  ...
  const flushList = () => {
    ...
    blocks.push(
      <ul key={`ul-${blocks.length}`} className="mb-4 list-disc space-y-1 pl-6">
        {list.map((item, i) => (
  ...
  const flushQuote = () => {
    ...
    blocks.push(
      <blockquote
        ...
        className="mb-4 border-l-4 border-accent/40 pl-4 italic"
      >
  ...
  lines.forEach((line) => {
      ...
      if (inCode)
        blocks.push(
          <pre
            ...
            className="mb-4 overflow-x-auto rounded-lg bg-black/5 p-4 text-sm"
          >
            <code>{code.join("\n")}</code>
          </pre>,
        );
    ...
    if (heading) {
      ...
      blocks.push(
        React.createElement(
          Heading,
          { key: `h-${blocks.length}`, className: "mb-3 mt-6 font-semibold" },
