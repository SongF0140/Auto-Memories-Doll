...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (error instanceof TemplateConflictError) {
      return NextResponse.json(apiError(ErrorCode.PROMPT_CREATE_FAILED, `模板已存在: ${error.message}`), { status: 409 });
    }
    return NextResponse.json(apiError(ErrorCode.PROMPT_CREATE_FAILED, (error as Error).message), { status: 500 });
  }
}
