...
export default function KnowledgeDetailPage() {
  ...
  const handleDeleteAll = useCallback(async () => {
    if (
      !confirm(
        `确定要删除"${topicMeta?.title}"下的所有 ${currentMemories.length} 条记忆吗？此操作不可恢复！`,
      )
    ) {
      return;
    }
        ...
        <div className="max-w-4xl mx-auto p-8 md:p-12">
          ...
          <header className="mb-8 pb-6 border-b-2 border-[#E8DCC8]">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h1
                  className="text-3xl md:text-4xl font-bold font-mono tracking-tight mb-2"
                  style={{ color: "#3E3224" }}
                >
                  {topicMeta.title}
                </h1>
            ...
            {currentMemories.map((memory, index) => (
              ...
              >
                ...
                <div className="pl-11">
                  ...
                  {/* 元数据 */}
                  <div
                    className="mt-4 pt-3 border-t border-dashed border-[#E8DCC8] flex items-center gap-4 text-xs font-mono"
                    style={{ color: "#A09585" }}
                  >
          ...
          {currentMemories.length > 1 && (
            <section className="mt-12 p-6 rounded-xl bg-gradient-to-br from-[#FFFEF9] to-[#FFF9ED] border-2 border-[#E8DCC8]">
              <h2
                className="font-mono font-bold text-lg mb-4 flex items-center gap-2"
                style={{ color: "#3E3224" }}
              >
                <span>✨</span> AI 知识总结
              </h2>
              <div className="prose prose-sm max-w-none" style={{ color: "#5D4E37" }}>
                <p className="leading-relaxed">
                  本知识点共收录 <strong>{currentMemories.length}</strong> 条相关记忆， 覆盖了{" "}
                  <strong>{topicMeta.tags.length}</strong> 个核心标签。 这些内容主要围绕{" "}
                  <strong>{topicMeta.title}</strong> 展开， 是您在
                  {
                    {
                      knowledge: "学习成长",
                      work: "工作实践",
                  ...
                  <div className="p-3 rounded-lg bg-white/60">
                    ...
                    <div className="flex flex-wrap gap-1 mt-2">
                      {topicMeta.tags.slice(0, 6).map((tag) => (
                        <span
                          key={tag}
                          className="px-2 py-0.5 rounded text-xs bg-[#C9A227]/10 text-[#8B7355]"
                        >
                          {tag}
                        </span>
                      ))}
