...
在采集与检索之上，系统还带一层夜间治理（每天凌晨 2:00 自动运行）：

- **结构化 Lint** — 死链、孤儿页、残缺卡、重复来源扫描（零 LLM 调用），结果进审计面板
- **幻觉防护** — 对话推断出的内容强制标记 `inference` 进人工队列；排序降权；注入上下文时标注"AI 推断，未经证实"
- **Supersession 取代链** — 知识被更新时旧主张不删除，标记 `superseded` 保留"曾经以为 X 后来发现是 Y"的完整轨迹
- **置信度** — 每条记忆带 `confidence`（0-1），随时间按类型差异化衰减、随你的访问强化
- **控制面** — 夜跑生成 `index.md` / `overview.md` / `log.md` / `review_q.md`，对话时作为模型的第一个导航点
- **Synthesis 编译** — 同话题 ≥5 张卡片自动编译成一页综合结论（`notes/synthesis/`），带来源引用与增量缓存
- **WiCER 编译验证** — 编译产物先过"诊断探针"验证（至多 2 轮定向重编译），验证不过不落盘
- **自适应检索路由** — 查询先分类再路由：单跳走向量+关键词 RRF 融合，多跳走图谱扩展，总览直接读编译产物（零 embedding 调用）
- **双信号热度** — 你的点击是强信号，自动召回只是弱信号，冷启动知识也有机会被看到
- **段级增量** — 追加型文件只处理新增部分，不重复消耗 Embedding/LLM 配额
- **window-use 检索键** — 每条记忆记录"什么场景下有用"，用它匹配你的查询，从源头提升召回
- **Locomo 基准** — `npm run eval:locomo` 跑三组对照（裸模型 / 现行 RAG / 改进后）出报告
